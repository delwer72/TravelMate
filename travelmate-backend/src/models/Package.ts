...
export const getAllPackages = async (filters: {
  ...
  else if (filters.sort === 'rating') sortObj = { ratingsAverage: -1 };
  else if (filters.sort === 'popular') sortObj = { ratingsQuantity: -1 };
...
export const getPackageById = (id: string): Promise<WithId<IPackage> | null> => {
  if (!ObjectId.isValid(id)) return Promise.resolve(null);
  return packagesCollection().findOne({ _id: new ObjectId(id) });
};

export const getPackageBySlug = (slug: string): Promise<WithId<IPackage> | null> => {
