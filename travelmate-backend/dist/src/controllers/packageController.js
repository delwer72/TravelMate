...
export const getPackages = async (req, res) => {
    try {
        ...
        if (count === 0) {
            await packagesCollection().insertMany(INITIAL_PACKAGES.map((pkg) => ({
                ...pkg,
...
export const getSinglePackage = async (req, res) => {
    try {
        ...
        if (!pkg) {
            ...
            const found = INITIAL_PACKAGES.find((p) => p.slug === target || p.title.toLowerCase() === target.toLowerCase());
            if (found) {
...
export const updateExistingPackage = async (req, res) => {
    try {
...
export const deleteExistingPackage = async (req, res) => {
    try {
        const id = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
